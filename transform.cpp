...
#pragma omp parallel for
    ...
    FFT1D(temp, x, r); // 调用一维快速傅立叶变换算法

#pragma omp parallel for
    for (i = 0; i < N; i++)// 写入时序信息
        x[i] = complex<float> (x[i].real() / N, -x[i].imag() / N);

    delete temp;
}

// 二维傅里叶变换，x，y方向分别进行一维傅里叶变换
bool FFT2D(complex<float> *x, int width, int height, complex<float> *y)
{
    ...
    for (i = 0; i < w * 3; i++)
    {
        ...
        FFT1D(temp_x, temp_y, hp);// 调用一维快速傅立叶变换算法
...
// 重载函数，进行平移操作，相当于上下左右对半平移互换，不是翻转
...
void generateFilter(int w, int h, int r, ImageFilterType type, float *filter)
{
    ...
    for (c = 0; c<3; c++)
    {
        for(i = 0; i < h; i++)
        {
            for(j = 0; j < w; j++)
            {
                // aera1含义？向左上角平移(h/2,w/2)后坐标与图像原点确定的矩形面积，作用？
                ...
                case ImageFilterType::GaussainLowPass:
                    filter[c*pixel_num + i * w + j] = exp(-area1 / (2*r*r));// 高斯低通滤波器
                    break;
                case ImageFilterType::ButterworthLowPass:
                    filter[c*pixel_num + i * w + j] = 1 / (1 + pow(sqrt(area1)/r,  2*n));// Butterworth低通滤波器
                    break;
                case ImageFilterType::ButterworthHighPass:
                    filter[c*pixel_num + i * w + j] = 1 / (1 + pow(r/sqrt(area1),  2*n));// Butterworth高通滤波器
                    break;
                }
            }
        }
